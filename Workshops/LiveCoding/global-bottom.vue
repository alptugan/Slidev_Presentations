...
<template>
    <div v-if="$slidev.nav.currentLayout !== 'intro-image'">
        <div
        ...
        >
            <div
                class="clip bg-gradient-to-r from-[#cccccc] to-white/10"
                :style="{ 'clip-path': `polygon(${poly1})`, opacity: opacity }"
            />
            <div
                class="clip bg-gradient-to-l from-[#fefefe] to-white/10"
                :style="{ 'clip-path': `polygon(${poly2})`, opacity: opacity }"
            />
            <div
                class="clip bg-gradient-to-t from-[#ffffff] to-white/10"
                :style="{ 'clip-path': `polygon(${poly3})`, opacity: 0.2 }"
            />
        </div>

        <footer
            class="text-shadow-sm text-shadow-color-#000000 text-color-#ffffff op-25 absolute bottom-0 left-0 right-0 p-2 z-10 text-2"
        >
            RAW (Alp Tuğan & Selçuk Artut)
            <span
                color-white
                op-100
                >•</span
            ...
            <div
                ...
                class="absolute bottom-1 w-9 text-2 right-1 p-1 z-99999999 bg-#000000aa rounded-md"
            >
